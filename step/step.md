...
```
选题名称
基于智能手机摄像头与接触声信号的轻量多模态表面材质识别技术研究

研究目标描述
本课题研究利用智能手机摄像头采集物体表面图像、利用麦克风采集触碰/敲击产生的短时接触声信号，构建图像-声音多模态融合模型，实现接触表面材质的实时识别。进一步研究小样本快速适配机制，使模型在新设备、新场景下可迁移、可快速校准，并完成端侧低延迟部署，为机器人操作与具身智能交互提供实时可靠的材质感知能力。

主要研究内容
（1）多模态材质表征学习：构建从单帧图像与短时接触声信号中联合学习表征的轻量网络，实现高精度材质分类。
（2）新设备与新场景快速适配：研究基于少量标注样本的快速适配方法，提升模型在跨设备、跨环境条件下的识别准确率与稳定性。
（3）低延迟端侧部署优化：对预处理、融合推理与反馈链路进行端侧优化，实现端到端延迟低于 50ms 的实时交互体验。
```
